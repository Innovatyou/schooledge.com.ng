...
<section class="panel full-screen">
	...
	<div class="row qrcode-scan">
	    ...
	    <div class="form-group box mt-md">
	        ...
	        <div class="mt-sm">
	        	<label for="hardware_scan_input" class="text-muted"><i class="fas fa-keyboard"></i> <?=translate('or_scan_with_a_hardware_scanner')?></label>
	        	<input type="text" class="form-control" id="hardware_scan_input" autocomplete="off" placeholder="<?=translate('hardware_scanner_input')?>">
	        </div>
	    ...
	    <div class="col-md-8 col-sm-12 mt-md">
				...
				<div class="row">
					<div class="col-md-12 mt-md">
						...
						<div class="export_title"><?php echo translate('attendance') . " " . translate('list'); ?></div>
...
<script type="text/javascript">
	...
	// Shared by the camera reader and the hardware-scanner input below — a USB/Bluetooth
	// HID scanner just types the decoded QR text into whichever field is focused.
	const processScannedCode = (decodedText, fromCamera) => {
	...
	$('#hardware_scan_input').trigger('focus').on('keypress', function (e) {
		if (e.which === 13) {
			e.preventDefault();
			var decodedText = $(this).val().trim();
			$(this).val('');
			if (decodedText) {
				processScannedCode(decodedText, false);
			}
		}
	});
	...
	$('.btn-confirm').on('click', function () {
		...
		$.ajax({
			...
			complete: function (data) {
				btn.button('reset');
				$.magnificPopup.close();
				$('#hardware_scan_input').trigger('focus');
			},
			error: function () {
				btn.button('reset');
			}
